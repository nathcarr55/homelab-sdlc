#!/usr/bin/env python3
"""
Review agent — uses Claude via LiteLLM to audit AI-generated code changes
against the original issue spec. Outputs 'approved' and 'feedback' for CI.
"""
...
client = OpenAI(base_url=f"{LITELLM_URL}/v1", api_key=LITELLM_KEY)
...
".join(lines[1:-1] if lines[-1].strip() == "```" else lines[1:])
    ...
    print(f"ERROR calling LiteLLM ({e}) — defaulting to approved.")
